...
class BigQueryClient(object):
    def __init__(self, decorator, http):
        """Creates the BigQuery client connection"""
        self.query_http = http
        self.service = build('bigquery', 'v2', http=self.query_http)
        self.decorator = decorator

    def getTableData(self, project, dataset, table):
        decorated_http = self.decorator.http()
        tablesCollection = self.service.tables()
        request = tablesCollection.get(projectId=project, datasetId=dataset, tableId=table)
